...
func (ec *executionContext) resolveEntity(
	...
	case "ResourceProvider":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "ResourceProvider": %w`, err)
		}
		switch resolverName {
		...
		case "findResourceProviderByID":
			id0, err := ec.unmarshalNID2goᚗinfratographerᚗcomᚋxᚋgidxᚐPrefixedID(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findResourceProviderByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindResourceProviderByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "ResourceProvider": %w`, err)
			}
...
func entityResolverNameForResourceProvider(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for ResourceProvider", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for ResourceProvider", ErrTypeNotFound))
			break
		}
		return "findResourceProviderByID", nil
	}
	return "", fmt.Errorf("%w for ResourceProvider due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
